...
test('buildPromptText centralizes provider strategy differences', () => {
  const questionData = {
  ...
  const chatgptPrompt = buildPromptText(questionData, 'chatgpt');
  const geminiPrompt = buildPromptText(questionData, 'gemini');

  assert.equal(chatgptPrompt.includes('compact JSON'), true);
  assert.equal(geminiPrompt.includes('valid JSON'), true);
  assert.equal(chatgptPrompt.includes('Please provide your answer in JSON format'), true);
});
